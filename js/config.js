// =============================================================================
// TrafficFlow Dashboard — Configuration & Mock Data
// =============================================================================

const CONFIG = {
  APP: { NAME: 'TrafficFlow', VERSION: '1.0.0', CURRENCY: 'BRL', LOCALE: 'pt-BR' },

  // Tokens NUNCA ficam aqui — são carregados via Settings e salvos no localStorage
  API: {
    MODE: 'mock', // 'mock' | 'facebook'
    FACEBOOK: {
      BASE_URL: 'https://graph.facebook.com/v19.0',
      TOKEN: null, // localStorage.getItem('tf_fb_token')
      VERSION: 'v19.0',
    },
    AI_ASSISTANT: {
      BASE_URL: null,        // Ex: 'https://api.anthropic.com/v1/messages'
      TOKEN: null,           // localStorage.getItem('tf_ai_token')
...
function _genDay(cfg, daysBack) {
  ...
  const freq        = parseFloat((impressions / Math.max(1,reach)).toFixed(2));
  ...
  const cpc_conv    = conversations > 0 ? parseFloat((spend / conversations).toFixed(2)) : 0;

  return {
    date: d.toISOString().split('T')[0],
    spend, impressions, reach, link_clicks, freq,
    ctr:  link_clicks > 0 ? parseFloat((link_clicks / impressions * 100).toFixed(2)) : 0,
    cpc, cpm: cpmA,
    leads, cpl, conversations,
...
function sumMetrics(daily) {
  ...
  t.freq = t.reach > 0 ? parseFloat((t.impressions / t.reach).toFixed(2)) : 0;
  return t;
}
...
function buildClient(meta, campaigns, metricsConfig) {
  ...
  const daily14 = daily30.slice(-14);

  campaigns.forEach(c => {
    c.daily = generateDailyData({
      ...metricsConfig,
      dailyBudget: metricsConfig.dailyBudget * (c._budgetShare || 0.33),
    }, 30);
...
const MOCK_CLIENTS = [
  buildClient(
    {
      ...
      adAccount: { id: 'act_112233445', name: 'Bella Vita Ads', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
    ...
    [
      { id:'c-bv-1', name:'Leads - Botox & Harmonização', status:'ACTIVE', objective:'LEAD_GENERATION', _budgetShare:0.50,
        adSets:[
          { id:'as-bv-1-1', name:'Mulheres 28-55 - Grande SP', status:'ACTIVE' },
  ...
  buildClient(
    {
      ...
      adAccount: { id: 'act_223344556', name: 'FitMax Anúncios', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
      contact: { name: 'Carlos Drummond', email: 'carlos@fitmax.com.br', phone: '(21) 99988-0002' },
      startDate: '2024-03-01',
      notes: 'Foco em matrículas no início do mês. Pico de conversão em jan e jul.',
    },
    [
      { id:'c-fm-1', name:'Matrículas - Planos Mensais',   status:'ACTIVE', objective:'LEAD_GENERATION', _budgetShare:0.55,
        adSets:[
          { id:'as-fm-1-1', name:'Homens e Mulheres 20-45 - RJ',   status:'ACTIVE' },
          { id:'as-fm-1-2', name:'Interesse em Academia/Fitness', status:'ACTIVE' },
        ]
      },
  ...
  buildClient(
    {
      ...
      adAccount: { id: 'act_334455667', name: 'Horizonte Imóveis Ads', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
      contact: { name: 'Ricardo Alves', email: 'ricardo@horizonte.com.br', phone: '(11) 3333-0003' },
      startDate: '2023-09-10',
      notes: 'Maior orçamento da carteira. Lançamento do Edifício Paramount previsto para jun/2025.',
    },
    [
      { id:'c-hz-1', name:'Lançamento - Paramount Residências', status:'ACTIVE', objective:'LEAD_GENERATION', _budgetShare:0.45,
        adSets:[
          { id:'as-hz-1-1', name:'Renda A/B - SP Capital',          status:'ACTIVE' },
      ...
      { id:'c-hz-2', name:'Unidades Prontas - Entrega Imediata', status:'ACTIVE', objective:'CONVERSIONS',     _budgetShare:0.35,
        adSets:[
          { id:'as-hz-2-1', name:'Interessados em Imóveis 60-100m²', status:'ACTIVE' },
          { id:'as-hz-2-2', name:'Remarketing - Visitas LP',          status:'ACTIVE' },
        ]
      },
  ...
  buildClient(
    {
      id: 'procurso',
      name: 'Escola Técnica ProCurso',
      niche: 'Educação',
      status: 'active',
      initials: 'PC',
      color: '#6366f1',
      budget: { monthly: 11000, daily: 367 },
      adAccount: { id: 'act_445566778', name: 'ProCurso Education Ads', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
    ...
    [
      { id:'c-pc-1', name:'Captação - Cursos Técnicos EAD', status:'ACTIVE', objective:'LEAD_GENERATION', _budgetShare:0.50,
        adSets:[
          { id:'as-pc-1-1', name:'18-35 anos - Interior MG/SP', status:'ACTIVE' },
          { id:'as-pc-1-2', name:'Desempregados e em Transição', status:'ACTIVE' },
        ]
      },
  ...
  buildClient(
    {
      ...
      adAccount: { id: 'act_556677889', name: 'TechStore Facebook Ads', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
      contact: { name: 'Felipe Rocha', email: 'felipe@techstore.com.br', phone: '(11) 97654-0005' },
      startDate: '2023-06-20',
      notes: 'E-commerce de gadgets e eletrônicos. Pico em Black Friday e Natal.',
    },
    [
      { id:'c-ts-1', name:'Catálogo Dinâmico - Remarketing', status:'ACTIVE', objective:'CONVERSIONS',     _budgetShare:0.40,
        adSets:[
          { id:'as-ts-1-1', name:'Visitantes 7 dias - Todos produtos', status:'ACTIVE' },
          { id:'as-ts-1-2', name:'Carrinhos Abandonados - 3 dias',     status:'ACTIVE' },
        ]
      },
      { id:'c-ts-2', name:'Prospecção - Smartphones',       status:'ACTIVE', objective:'CONVERSIONS',     _budgetShare:0.35,
        adSets:[
          { id:'as-ts-2-1', name:'Interesse Tecnologia 20-45',         status:'ACTIVE' },
          { id:'as-ts-2-2', name:'Lookalike Compradores',              status:'ACTIVE' },
        ]
      },
  ...
  buildClient(
    {
      id: 'silva-adv',
      name: 'Silva & Associados Adv.',
      niche: 'Jurídico',
      status: 'paused',
      initials: 'SA',
      color: '#8b5cf6',
      budget: { monthly: 5500, daily: 183 },
      adAccount: { id: 'act_667788990', name: 'Silva Advocacia Ads', currency: 'BRL', status: 'ACTIVE', timezone: 'America/Sao_Paulo' },
      contact: { name: 'Dr. André Silva', email: 'andre@silvaadv.com.br', phone: '(11) 3232-0006' },
      startDate: '2024-04-01',
      notes: 'Foco em Direito Trabalhista e Previdenciário. Campanhas pausadas aguardando revisão criativa.',
    },
    [
...
// Carrega token do localStorage se disponível
(function loadTokens() {
  ...
  if (aiToken) CONFIG.API.AI_ASSISTANT.TOKEN = aiToken;
